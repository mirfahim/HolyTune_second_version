  ...
  Future<void> fetchComments() async {
    ...
    try {
      final response = await http.post(Uri.parse(ApiUrl.loadcomments),
          headers:headers ,
          body:
            {"id": 0.toString(), "media": media.toString()}

      );
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.
       print("my response for fetching comments is ${response.statusCode}");
        Map<String, dynamic> res = json.decode(response.body);
        print("my comment is $res");
  ...
  Future<void> constructComment(String content) async {
    try {
      ...
      final response = await http.post(Uri.parse(ApiUrl.makecomment),
          body: {"content": Utility.getBase64EncodedString(content),
          "email": SharedPref.profileEmail,//userdata.email,
          "media": media.toString()});
      if (response.statusCode == 200) {
        // If the server did return a 200 OK response,
        // then parse the JSON.

        print("This is my comment upload " + "${response.statusCode}");
        ...
        if (_status == "ok") {
          ...
          print("This is my comment response " + "${response.statusCode}");
          makeCommentsError();
          print("one");
        }
      } else {
